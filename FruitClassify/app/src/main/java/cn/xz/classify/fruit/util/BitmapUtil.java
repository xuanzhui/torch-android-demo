...
public class BitmapUtil {
    ...
    public static Bitmap centerCrop(Bitmap original, int targetWidth, int targetHeight) {
        int startX;
        int width;
        if (targetWidth > original.getWidth()) {
            startX = 0;
            width = original.getWidth();
        } else {
            startX = (original.getWidth() - targetWidth) / 2;
            width = targetWidth;
        }

        int startY;
        int height;
        if (targetHeight > original.getHeight()) {
            startY = 0;
            height = original.getHeight();
        } else {
            startY = (original.getHeight() - targetHeight) / 2;
            height = targetHeight;
        }

        return Bitmap.createBitmap(original, startX, startY, width, height);
    }
}